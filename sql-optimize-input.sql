...
SELECT 
    ...
    LPAD(EKPO.BSART,6,0) as BSART, -- should be "EKPO.BSART as BSART"? - dont see the reasson why is there (EKPO.BSART,6,0)
	BKPF.GJAHR, 
	BSEG.BUKRS, 
	BSEG.BUZEI, 
	BSEG.BSCHL, 
...
LEFT JOIN "DTAG_DEV_CSBI_CELONIS_WORK"."dtag.dev.csbi.celonis.app.p2p_elog::__P2P_REF_CASES" AS EKPO ON 1=1
    AND RSEG.ZSYSNAME = EKPO.SOURCE_SYSTEM -- instead of "AND" should be "WHERE"
...
INNER JOIN "DTAG_DEV_CSBI_CELONIS_DATA"."dtag.dev.csbi.celonis.data.elog::V_BKPF" AS BKPF ON 1=1
    ...
    AND RSEG.MANDT in ('200') -- instead od "in('200') I will use =200
